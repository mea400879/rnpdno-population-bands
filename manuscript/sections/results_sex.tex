...
As documented in Section~\ref{sec:descriptive}, the four outcome statuses differ substantially in their sex composition.
...
\input{tables/table7_sex_hh_counts.tex}
...
The moderate Jaccard values indicate that roughly half of the hotspot geography is shared, while the remainder is sex-specific, producing partially overlapping but not identical recovery geographies.
...
The low overlap indicates that the few municipalities with concentrated female Not Located cases are not the same municipalities driving male Not Located clustering.
Even if the female HH set were expanded to match the male set size, the 28 shared municipalities would represent only 33\% of the male set, suggesting that the spatial divergence is not solely a power artifact.

For Located Dead, 27 municipalities are classified as male HH and zero as female HH (Jaccard = 0.000).
This reflects a statistical power limitation rather than a substantive finding about gendered victimization geography.
